  ...
  const controller = {
    ...
    async background() { await tail; if (state.status === 'Not saved') await controller.retry(); },
      ...
      finalising = (async () => {
        ...
        if (state.status === 'Not saved') await controller.retry();
    ...
    async discard() {
      ...
      ++generation;
      update({ busy: true, error: null });
      try { await tail; await (await deps.repository()).discardDraft(id); update({ writing: null, text: '', status: 'Saved draft' }); }
      catch { update({ error: 'Could not discard the draft. Please try again.' }); }
