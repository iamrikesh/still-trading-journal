...
Continued from published `b1a471f` on `codex/android-device-setup`; GitHub matched the clean checkout before work. See the [longer memory-baseline review](reviews/2026-09-23-recording-memory-baseline.md). App/native source is unchanged. This checkpoint supersedes the resource-run handoff below.
...
- **Current device state:** the phone is locked; Rikesh has been asked to unlock it. The same app process was present at follow-up. Vault inspection found 15 files, no staging/playback plaintext and no pending ciphertext. **Pending database state and recovery after the failed start are not yet verified.** No unknown intent was discarded. Microphone permission is still granted; later `/data` free-space reading was 1008432 KiB, above the native 100 MiB admission threshold. These later readings do not identify the cause at failure time.
- **Resume first:** unlock the S20, foreground still. and inspect **Cycle test Sep23** before starting another recording. Confirm native state and pending count. If a pending synthetic intent exists, inspect staging and exercise normal Retry/Discard as appropriate; preserve the earlier four plus seven lifecycle samples. Diagnose/reproduce the failed start before editing app code. A resumed test is a separate measurement segment, not an uninterrupted twenty-cycle pass.
...
- **Next after recovery:** finish a bounded retest with exact UI-state assertions; coordinate call/headphone interruption when available. Actual Deny-button/navigation behavior, device low-space/save-fault cases and the existing release gates remain open.
- **Exercise:** allocated native memory fell about 15.3 MiB during one cycle while total PSS fell about 6.6 MiB. Explain why those are different measurements, and why memory evidence does not resolve the separate failed-start behavior. See Lesson6 and the dated review.
