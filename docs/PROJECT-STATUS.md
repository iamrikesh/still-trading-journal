...
- **Stopped and verified:** Metro's port8081 listener is absent, still. has no running phone process, USB port8081 forwarding is removed, and no Java build process remains. Original phone sleep setting is restored. Existing journals, keys, three voice samples, the installed app, one local APK and reusable toolchains/caches are retained.
...
- Reused checkout/toolchains, S: alias and caches; no clean build, emulator or APK copies. Latest disk: **13.71 GiB free**, initial 13.75 GiB. During testing Metro used IPv4 port8081 and two workers; it is now stopped as described above. The scoped Kotlin workaround applies at task-graph-ready time; see Lesson3.
